...
kb_res, doc_res, hybrid_res, ensemble_res, ensemble_all_res = None,None,None,None,None
...
def compare_pr(kb_pred_file, doc_pred_file, hybrid_pred_file, test_file, w_kb, eps_doc, eps_kb, eps_ensemble, eps_hybrid, eps_ensemble_all):
    print(doc_res,kb_res,hybrid_res)
    assert doc_res != None
    ...
    with open(kb_pred_file) as f_kb, open(doc_pred_file) as f_doc, open(hybrid_pred_file) as f_hybrid, open(test_file) as f_test:
        line_id = 0
        for line_kb, line_doc, line_hybrid, line_test in tqdm(zip(f_kb, f_doc, f_hybrid,f_test)):
            line_id += 1
            ...
            doc_res.write(json.dumps({'id': line_test['id'], 'question': line_test['question'], 'answers': line_test['answers'], \
                'seeds': line_doc['seeds'], 'dist': line_doc['dist'], \
                'p': p, 'r':r, 'f1': f1, 'hits': hits}) + '\n')
            ...
            kb_res.write(json.dumps({'id': line_test['id'], 'question': line_test['question'], 'answers': line_test['answers'], \
                'seeds': line_kb['seeds'], 'dist': line_kb['dist'], \
                'p': p, 'r':r, 'f1': f1, 'hits': hits}) + '\n')
            ...
            ensemble_res.write(json.dumps({'id': line_test['id'], 'question': line_test['question'], 'answers': line_test['answers'], \
                'seeds': line_doc['seeds'], 'dist': line_doc['dist'], \
                'p': p, 'r':r, 'f1': f1, 'hits': hits}) + '\n')
            ...
            hybrid_res.write(json.dumps({'id': line_test['id'], 'question': line_test['question'], 'answers': line_test['answers'], \
                'seeds': line_hybrid['seeds'], 'dist': line_hybrid['dist'], \
                'p': p, 'r':r, 'f1': f1, 'hits': hits}) + '\n')
            ...
            ensemble_all_res.write(json.dumps({'id': line_test['id'], 'question': line_test['question'], 'answers': line_test['answers'], \
                'seeds': line_doc['seeds'], 'dist': line_doc['dist'], \
                'p': p, 'r':r, 'f1': f1, 'hits': hits}) + '\n')
...
if __name__ == "__main__":
    ...
    kb_res = open(sys.argv[6],'w')
    doc_res = open(sys.argv[7],'w')
    hybrid_res = open(sys.argv[8],'w')
    print(sys.argv[8])
    ensemble_res = open(sys.argv[9],'w')
    ensemble_all_res = open(sys.argv[10],'w')
    ...
    compare_pr(pred_kb_file, pred_doc_file, pred_hybrid_file, test_file, w_kb, eps_doc, eps_kb, eps_ensemble, eps_hybrid, eps_ensemble_all)
