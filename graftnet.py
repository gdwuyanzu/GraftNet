...
class GraftNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        #query_hidden_emb:(batch_size,max_query_word,entity_dim) query_node_emb:(1,batch_size,entity_dim)
        ...
        if self.use_kb:
            ...
            #entity_dim是query中单词att系数加权之后query的表示，W的每一行比表示query下，每个fact和问题的相似度
        ...
        if self.use_doc:
            ...
            # document_textual_emb : batch_size * max_relevant_doc, max_document_word, entity_dim * 2 document_node_emb : 2,batch_size * max_relevant_doc,entity_dim
        ...
        for i in range(self.num_layer):
            ...
            # document -> entity
            if self.use_doc:
                # entity_pos_mat(batch_size, max_local_entity, max_relevant_doc*max_doc_word) e2d_out_dim(batch_size, max_local_entity, 1)，pagerank_e2d是entity到每个pos的pr值
            ...
            # fact -> entity
            if self.use_kb:
                # local_fact_emb(batch_size, max_fact, entity_dim) entity2fact_mat(batch_size, max_fact, max_local_entity) local_entity_emb(batch_size, max_local_entity, entity_dim)
                ...
                # W_tilde(batch_size, max_fact) e2f_softmax(batch_size, max_local_entity) e2f_softmax_normalized是每个fact边上传递的加权的pr值
        ...
        query_hidden_emb = torch.sum(query_hidden_emb,dim=1) / torch.sum(query_mask,dim=1,keepdim=True)
        entity_question_emb = torch.cat((local_entity_emb,query_hidden_emb),dim=1)
        ...
        score = self.score_func(self.linear_drop(entity_question_emb)).squeeze(dim=2) # batch_size, max_local_entity
